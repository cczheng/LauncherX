...
public class ArrowTipView extends AbstractFloatingView {
    ...
    public ArrowTipView show(String text, int gravity, int arrowMarginStart, int top) {
        ...
        LayoutParams lp = (LayoutParams) mArrowView.getLayoutParams();
        lp.gravity = gravity;
    ...
    private void updateArrowTipInView() {
        ...
        if (mIsPointingUp) {
            ...
            ((MarginLayoutParams) arrowLp).setMargins(0, 0, 0, -1 * arrowTipRadius);
        } else {
            addView(mArrowView, 1);
            ((MarginLayoutParams) arrowLp).setMargins(0, -1 * arrowTipRadius, 0, 0);
        }
    }
