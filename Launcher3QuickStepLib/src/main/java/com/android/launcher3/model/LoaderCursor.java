...
public class LoaderCursor extends CursorWrapper {
    ...
    public LoaderCursor(Cursor cursor, Uri contentUri, LauncherAppState app,
        ...
        // Init column indices
        iconIndex = getColumnIndexOrThrow(Favorites.ICON);
        iconPackageIndex = getColumnIndexOrThrow(Favorites.ICON_PACKAGE);
        iconResourceIndex = getColumnIndexOrThrow(Favorites.ICON_RESOURCE);
        titleIndex = getColumnIndexOrThrow(Favorites.TITLE);

        idIndex = getColumnIndexOrThrow(Favorites._ID);
        containerIndex = getColumnIndexOrThrow(Favorites.CONTAINER);
        itemTypeIndex = getColumnIndexOrThrow(Favorites.ITEM_TYPE);
        screenIndex = getColumnIndexOrThrow(Favorites.SCREEN);
        cellXIndex = getColumnIndexOrThrow(Favorites.CELLX);
        cellYIndex = getColumnIndexOrThrow(Favorites.CELLY);
        profileIdIndex = getColumnIndexOrThrow(Favorites.PROFILE_ID);
        restoredIndex = getColumnIndexOrThrow(Favorites.RESTORED);
        intentIndex = getColumnIndexOrThrow(Favorites.INTENT);
    }
    ...
    public boolean commitDeleted() {
        if (itemsToRemove.size() > 0) {
            ...
            mContext.getContentResolver().delete(mContentUri, Utilities.createDbSelectionQuery(
                    Favorites._ID, itemsToRemove), null);
            return true;
        }
        return false;
    }
        ...
        if (restoredRows.size() > 0) {
            ...
            mContext.getContentResolver().update(mContentUri, values,
                    Utilities.createDbSelectionQuery(
                            Favorites._ID, restoredRows), null);
        }
    }
    ...
    protected boolean checkItemPlacement(ItemInfo item) {
        ...
        if (item.container == Favorites.CONTAINER_HOTSEAT) {
            final GridOccupancy hotseatOccupancy =
                    occupied.get(Favorites.CONTAINER_HOTSEAT);
            ...
            if (hotseatOccupancy != null) {
                ...
                occupied.put(Favorites.CONTAINER_HOTSEAT, occupancy);
                return true;
            }
